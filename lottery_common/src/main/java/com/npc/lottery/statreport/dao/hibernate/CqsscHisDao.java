 ...
 * 
 */
public class CqsscHisDao extends HibernateDaoSupport implements ICqsscHisDao {
    ...
    public ArrayList<CqsscHis> findCqsscHis(String conditionStr) {
        ...
        sqlInfo.append("COUNT(*) OVER(PARTITION BY order_no) recordNum FROM TB_CQSSC_HIS t WHERE 1=1 ");
        ...
        Session session = getHibernateTemplate().getSessionFactory()
                .getCurrentSession();
        //List resultTemp = session.createSQLQuery(sqlInfo.toString()).addEntity(CqsscHis.class).list();
        List resultTemp = session.createSQLQuery(sqlInfo.toString())
                .addEntity("t", CqsscHis.class).addScalar("recordNum").list();
        ...
        if (null == resultTemp || resultTemp.size() < 1) {
            ...
            result = new ArrayList<CqsscHis>();
            for (int i = 0; i < resultTemp.size(); i++) {
                entity = (CqsscHis) ((Object[]) resultTemp.get(i))[0];
    ...
    public List findCqsscHisList(final ConditionData conditionData,
        ...
        return getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session)
                ...
                Criteria criteria = session.createCriteria(CqsscHis.class);
