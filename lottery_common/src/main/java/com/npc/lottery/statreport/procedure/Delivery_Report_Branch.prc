...
BEGIN
     ...
     BEGIN
          ...
          FOR stockholder_pos IN stockholder_cursor LOOP
              ...
              -- 2.1.4 调用股东交收报表存储过程
              Delivery_Report_Stockholder(subID, lotteryType, playType, periodsNum, startDate, endDate, resultFlag, statReportStockholder);
              -- 读取代理交收报表存储过程所形成的数据
              declare
      ...
      /******** 分公司对应的直属会员数据 开始 ********/
      -- 调用直属会员交收报表存储过程
      Delivery_Report_Dir_Member(userID, '3', lotteryType, playType, periodsNum, startDate, endDate, resultFlag);
      -- 查询直属会员统计数据
      declare
