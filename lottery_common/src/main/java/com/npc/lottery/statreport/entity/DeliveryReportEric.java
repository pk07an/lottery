 ...
 * 
 */
public class DeliveryReportEric implements Serializable, Cloneable {
     ...
     * 克隆对象
     */
    public DeliveryReportEric clone() throws CloneNotSupportedException {
        return (DeliveryReportEric) super.clone();
    }
    ...
    public Double getOfferSuperior() {
        ...
        //如果是直属会员的话,就取直属会员的上一级用户类型再加1来处理
        if(MemberStaff.USER_TYPE_MEMBER.equals(userType) && !ManagerStaff.USER_TYPE_AGENT.equals(parentUserType)){
        	try {
        		opUserType = String.valueOf(Integer.valueOf(parentUserType)+1);
			} catch (Exception e) {
				//这里以后要处理一下异常
				opUserType = parentUserType;
			}
        }else{
        	opUserType = userType;
        }
        if (ManagerStaff.USER_TYPE_STOCKHOLDER
                .equalsIgnoreCase(opUserType)) {
            //因为是分公司进来查看股东的，所以是以分公司为标准，取分公司的上一级
            useRate = this.moneyRateChief;
        } else if (ManagerStaff.USER_TYPE_GEN_AGENT.equalsIgnoreCase(opUserType)) {
            useRate = this.moneyRateChief + this.moneyRateBranch;
        } else if (ManagerStaff.USER_TYPE_AGENT.equalsIgnoreCase(opUserType)) {
            useRate = this.moneyRateChief + this.moneyRateBranch + this.moneyRateStockholder;
        } else if (MemberStaff.USER_TYPE_MEMBER.equalsIgnoreCase(opUserType)) {
	...
	public java.sql.Date getBettingDate() {
		/*java.util.Date date = new java.sql.Date(this.bettingDate.getTime());
		System.out.println("~~~~~~~~"+this.bettingDate.getTime());
		java.sql.Date cDate = new Date(date.getTime()- 60 * 60 * 1000 * 3);
