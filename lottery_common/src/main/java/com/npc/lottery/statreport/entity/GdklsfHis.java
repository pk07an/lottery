...
/**
 * 功能实体类（投注历史（广东快乐十分）表 TB_GDKLSF_HIS）
 * 
 *
 */
public class GdklsfHis implements Serializable {
    ...
    private String typeCode;//投注类型（注意页面上的投注类型是commissionType）
    ...
    public double getWinAmount2() {
        ...
        if (WINNING_LOTTERY.equalsIgnoreCase(winState)) {
            ...
            amount = winAmount;
            //增加退水
            amount += (money * commissionMember.doubleValue() * 1.0) / 100;
        } else if (NOT_WINNING_LOTTERY.equalsIgnoreCase(winState)) {
            //未中奖则读取投注额
            amount = -1.0 * money;
            //增加退水
            amount = amount + (money * commissionMember.doubleValue() * 1.0) / 100;
