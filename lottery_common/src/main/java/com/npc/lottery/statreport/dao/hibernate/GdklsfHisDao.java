 ...
 * 
 */
public class GdklsfHisDao extends HibernateDaoSupport implements IGdklsfHisDao {
    ...
    public ArrayList<GdklsfHis> findGdklsfHis(String conditionStr) {
        ...
        sqlInfo.append("COUNT(*) OVER(PARTITION BY order_no) recordNum FROM tb_gdklsf_his t WHERE 1=1 ");
        ...
        Session session = getHibernateTemplate().getSessionFactory()
                .getCurrentSession();
        //List resultTemp = session.createSQLQuery(sqlInfo.toString()).addEntity(GdklsfHis.class).list();
        List resultTemp = session.createSQLQuery(sqlInfo.toString())
                .addEntity("t", GdklsfHis.class).addScalar("recordNum").list();
        ...
        if (null == resultTemp || resultTemp.size() < 1) {
            ...
            result = new ArrayList<GdklsfHis>();
            for (int i = 0; i < resultTemp.size(); i++) {
                entity = (GdklsfHis)((Object[])resultTemp.get(i))[0];
    ...
    public List findGdklsfHisList(final ConditionData conditionData,
        ...
        return getHibernateTemplate().executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session)
                ...
                Criteria criteria = session.createCriteria(GdklsfHis.class);
                ...
                new HibernateCallback() {
                    public Object doInHibernate(Session session)
                        ...
                        Criteria criteria = session
                                .createCriteria(GdklsfHis.class);
