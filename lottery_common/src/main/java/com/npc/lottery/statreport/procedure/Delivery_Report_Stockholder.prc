...
BEGIN
     ...
     BEGIN
          ...
          FOR gen_agent_pos IN gen_agent_cursor LOOP
              ...
              -- 2.1.4 调用总代理交收报表存储过程
              Delivery_Report_Gen_Agent(subID, lotteryType, playType, periodsNum, startDate, endDate, resultFlag, statReportGenAgent);
              -- 读取代理交收报表存储过程所形成的数据
              declare
      ...
      /******** 股东对应的直属会员数据 开始 ********/
      -- 调用直属会员交收报表存储过程
      Delivery_Report_Dir_Member(userID, '4', lotteryType, playType, periodsNum, startDate, endDate, resultFlag);
      -- 查询直属会员统计数据
      declare
