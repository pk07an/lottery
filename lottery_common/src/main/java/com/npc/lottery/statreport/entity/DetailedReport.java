...
public class DetailedReport {
    ...
    private double winAmountTotal;//会员输赢（原来的会员输赢，好像加了退水，现在直接使用winAmount TODO）
     ...
     */
    public DetailedReport(GdklsfHis gdklsfHisEntity) {
        //构造数据
        ...
        this.winAmount = gdklsfHisEntity.getWinAmount();
     ...
     */
    public DetailedReport(CqsscHis cqsscHisEntity) {
        //构造数据
        ...
        this.winAmount = cqsscHisEntity.getWinAmount();
     ...
     * @param cqsscHisEntity
     */
    public DetailedReport(HklhcHis hklhcHisEntity) {
        //构造数据
        ...
        this.winAmount = hklhcHisEntity.getWinAmount();
     ...
     * @param bjscHisEntity
     */
    public DetailedReport(BjscHis bjscHisEntity) {
        //构造数据
        ...
        this.winAmount = bjscHisEntity.getWinAmount();
     ...
     */
    public DetailedReport(LotInfoHis lotInfoHisEntity) {
        //构造数据
        ...
        this.winAmount = lotInfoHisEntity.getWinAmount();
     ...
     * 解析直属会员数据信息
     * 
     * @param parentUserType        父用户类型
     * @param dirMemeberDataList    会员报表数据
     * 
     * @return
     */
    public static int parseDirMemberData(String parentUserType,
            ArrayList<DetailedReport> dirMemeberDataList) {

        if (null == dirMemeberDataList) {
            return 0;
        }

        DetailedReport dirMemberEntity = null;
        int parseNum = 0;

        for (int i = 0; i < dirMemeberDataList.size(); i++) {
            try {
                dirMemberEntity = dirMemeberDataList.get(i);
                //根据父类型，赋值对应的上级占成
                if (ManagerStaff.USER_TYPE_CHIEF
                        .equalsIgnoreCase(parentUserType)) {
                    //总监
                    //占成
                    dirMemberEntity.rateAgentName = dirMemberEntity.rateChief
                            .longValue() + "%";
                    //您的结果
                    dirMemberEntity.resultTotal = (dirMemberEntity.resultTotalNoRate)
                            * dirMemberEntity.rateChief.longValue() / 100;
                } else if (ManagerStaff.USER_TYPE_BRANCH
                        .equalsIgnoreCase(parentUserType)) {
                    //分公司
                    //占成
                    dirMemberEntity.rateAgentName = dirMemberEntity.rateBranch
                            .longValue() + "%";
                    //您的结果
                    dirMemberEntity.resultTotal = (dirMemberEntity.resultTotalNoRate)
                            * dirMemberEntity.rateBranch.longValue() / 100;
                } else if (ManagerStaff.USER_TYPE_STOCKHOLDER
                        .equalsIgnoreCase(parentUserType)) {
                    //股东
                    //占成
                    dirMemberEntity.rateAgentName = dirMemberEntity.rateStockholder
                            .longValue() + "%";
                    //您的结果
                    dirMemberEntity.resultTotal = (dirMemberEntity.resultTotalNoRate)
                            * dirMemberEntity.rateStockholder.longValue() / 100;
                } else if (ManagerStaff.USER_TYPE_GEN_AGENT
                        .equalsIgnoreCase(parentUserType)) {
                    //总代理
                    //占成
                    dirMemberEntity.rateAgentName = dirMemberEntity.rateGenAgent
                            .longValue() + "%";
                    //您的结果
                    dirMemberEntity.resultTotal = (dirMemberEntity.resultTotalNoRate)
                            * dirMemberEntity.rateGenAgent.longValue() / 100;
                }
    ...
    public String getResultChief() {
        //计算结果值（会员结果*占成）
        double result = this.getResultTotal() * (this.rateChief.doubleValue())
                / 100;
        DecimalFormat df = new DecimalFormat("0.00");
    ...
    public String getResultBranch() {
        //计算结果值（会员结果*占成）
        double result = this.getResultTotal() * (this.rateBranch.doubleValue())
                / 100;
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(result);
    }
    ...
    public String getResultStockholder() {
        //计算结果值（会员结果*占成）
        double result = this.getResultTotal()
    ...
    public String getResultAgent() {
        //计算结果值（会员结果*占成）
        double result = this.getResultTotal() * (this.rateAgent.doubleValue())
                / 100;
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(result);
    }
    ...
    public String getCommissionBranchValue() {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(1.0 * moneyTotal * commissionBranch.doubleValue() / 100);
    }
    ...
    public String getCommissionGenAgentValue() {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(1.0 * moneyTotal * commissionGenAgent.doubleValue() / 100);
    }
    ...
    public String getCommissionStockholderValue() {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(1.0 * moneyTotal * commissionStockholder.doubleValue() / 100);
    }
    ...
    public String getCommissionAgentValue() {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(1.0 * moneyTotal * commissionAgent.doubleValue() / 100);
    }
    ...
    public String getCommissionMemberValue() {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(1.0 * moneyTotal * commissionMember.doubleValue() / 100);
    }
