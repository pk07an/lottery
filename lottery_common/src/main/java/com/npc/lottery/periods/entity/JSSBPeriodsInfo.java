...
public class JSSBPeriodsInfo implements Serializable {

	/**
	 * 
	 */
	private static final long	serialVersionUID	= 9094048478037321096L;
	...
	private java.lang.String	state;										// 盘期状态 盘期状态，取值如下：0 — 已禁用；1 — 未开盘；2 — 已开盘；3— 已封盘；4— 拿到开奖结果；7 — 已开奖。默认值为 1
																			// — 未开盘
	...
	public String getStateName() {
		if (this.state.equals(Constant.NOT_STATUS)) {
			stateName = "未开盘";
		}
		if (this.state.equals(Constant.OPEN_STATUS)) {
			stateName = "已开盘";
		}
		if (this.state.equals(Constant.STOP_STATUS)) {
			stateName = "已封盘";
		}
		if (this.state.equals(Constant.LOTTERY_STATUS)) {
			stateName = "拿到开奖结果";
		}
		if (this.state.equals(Constant.FAIL_STATUS)) {
			stateName = "获取开奖号码失败";
		}
		if (this.state.equals(Constant.INVALID_STATUS)) {
			stateName = "停开或作废";
		}
		if (this.state.equals(Constant.SCAN_SUC_STATUS)) {
			stateName = "系统兑奖成功";
		}
