...
$(document).ready(function() {
	...
	<s:if test="#request.selectGenAgent.rateRestrict==1">
		//如果上级总代理设置了限制占成数
		var maxRate = $('#jq_below_max_rate').val()
	</s:if>
	<s:elseif test="#request.selectGenAgent.stockholderStaffExt.rateRestrict==1">
		//如果上上级股东设置了限制占成数，本级最大占成应该为  股东限制占成-总代理占成
		var maxRate = $('#jq_below_max_rate').val()-${selectGenAgent.shareholderRate}
	</s:elseif>
	<s:else>
...
function enterAgain()
{
  var selAccount=$('#genAgentAccount').val();
  $('#jq_genAgent_account').val(selAccount);
  $('#changeForm').submit();	
	...
	function checkSubmit() {
		...
		<s:if test="#request.isGenAgentOrSub==false">
		var selAccount=$('#genAgentAccount').val();
		//add by peter
		...
		<s:if test="#request.selectGenAgent.rateRestrict==1">
			if(parseInt( $('#jq_below_max_rate').val())<parseInt($('#upperRate').val()))
			{
				alert("總代理占成不可高於"+ $('#jq_below_max_rate').val()+"% 請重新設定");
				$("#upperRate").focus();
	            return false;
			}
		</s:if>
		<s:elseif test="#request.selectGenAgent.stockholderStaffExt.rateRestrict==1">
			var maxRate = parseInt( $('#jq_below_max_rate').val()) - parseInt(${selectGenAgent.shareholderRate});
		...
		<s:else>
			...
			{
				alert("總代理占成不可高於"+(100-maxRate)+"% 請重新設定");
				$("#upperRate").focus();
	            return false;
			}
		</s:else>
		...
		if(item==1)
		 {
			...
			<s:if test="#request.selectGenAgent.rateRestrict==1">
				//如果上级总代理有设置限制占成数，代理设置的限制占成数=上级总代理限制占成-本级占成
				maxRc = parseInt($('#jq_below_max_rate').val()) - parseInt($('#upperRate').val());
			</s:if>
			<s:elseif test="#request.selectGenAgent.stockholderStaffExt.rateRestrict==1">
				//如果上上级股东有设置限制占成数，代理设置的限制占成数=上上级股东限制占成-上级总代理占成-本级占成
				maxRc = parseInt($('#jq_below_max_rate').val()) -parseInt(${selectGenAgent.shareholderRate})- parseInt($('#upperRate').val());
			</s:elseif>
...
<div class="content">
	<s:form id="sForm"
		action="%{pageContext.request.contextPath}/user/saveAgent.action?searchAccount=%{searchAccount}&parentAccount=%{parentAccount}&searchUserStatus=0&type=%{type}" method="post">
<table width="100%" border="0" >
          ...
          <tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="99%" align="left" class="F_bold">&nbsp;代理 -&gt; 新增</td>
              </tr>
            </table>
            </td>
 ...
 <table width="100%" border="0px" cellspacing="0" cellpadding="0" class="mt4 king higher">
  <tr>
    ...
    <s:if test="#request.isGenAgentOrSub==true">
      <td class="t_left"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="none_border">
      <tr>
        <td width="74"><input type="text" class="b_g" value="<s:property value="#request.selectGenAgent.account"/>" name="agentStaffExt.genAgentStaffExt.account" readonly="readonly"></input></td>
        <td><span>名稱：<s:property value="#request.selectGenAgent.chsName"/></span><span class="blue ml10">餘額：<s:property value="#request.selectGenAgent.availableCreditLine"/></span></td>
      </tr>
    </table></td>
    ...
    <td class="t_left"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="none_border">
      <tr>
        <td width="74"> <s:select id="genAgentAccount" name="agentStaffExt.genAgentStaffExt.account" list="#request.selList" listKey="account" listValue="account" onChange="enterAgain();" value="#request.selectGenAgent.account"></s:select></td>
        <td><span>名稱：<s:property value="#request.selectGenAgent.chsName"/></span><span class="blue ml10">餘額：<s:property value="#request.selectGenAgent.availableCreditLine"/></span></td>
      </tr>
    </table></td>
  ...
  <tr>
    ...
    <td width="82%" class="t_left"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="none_border">
      <tr>
        <td width="74"><input type="text" value="" class="" maxlength="20" id="account" name="agentStaffExt.account" onblur="javascript:checkShopsName();" maxlength="20"></input></td>
  ...
  <tr>
    ...
    <td class="t_left"><input name="agentStaffExt.chsName" type="text" value="" maxlength="20" /></td>
  </tr>
  <tr>
    <td class="t_right even4">信用額度</td>
    <td class="t_left"><input type="text"  id="credLine" name="agentStaffExt.totalCreditLine" value="0" maxlength="9" /><span id="jq_ch_money" style="color:red; padding-left: 5px; font-weight: bold;"></span> </td>
  </tr>
  <tr>
    <td class="t_right even4">總代理占成</td>
    <td class="t_left"><input name="agentStaffExt.genAgentRate" value="0" size="6" maxlength="10" id="upperRate"/>
      % &nbsp;最高可設占成&nbsp;
      <s:if test="#request.selectGenAgent.rateRestrict==1">
      	<s:property value="#request.selectGenAgent.belowRateLimit"/>
      </s:if>
      <s:elseif test="#request.selectGenAgent.stockholderStaffExt.rateRestrict==1">
      	<s:property value="#request.selectGenAgent.stockholderStaffExt.belowRateLimit-#request.selectGenAgent.shareholderRate"/>
      </s:elseif>
      <s:else>
      	<s:property value="100-#request.selectGenAgent.shareholderRate-#request.selectGenAgent.stockholderStaffExt.branchRate-#request.selectGenAgent.stockholderStaffExt.branchStaffExt.chiefRate"/>
  	  </s:else>
  	  %</td>
  </tr>
  ...
  <tr>
    ...
    <td class="t_left"><s:radio name="agentStaffExt.rateRestrict" id="rateRestrict" list="#{'0':' 占餘成數下線任占','1':' 限制下線可占成數'}" label="下線占成上限" value="0">
						</s:radio><input type="text" name="agentStaffExt.belowRateLimit" value="" id="jq_below_rate_limit" style="width:50px;"/>%
     </td>
  </tr>
  ...
  <tr>
    ...
    <s:radio name="agentStaffExt.replenishment" id="replenishment" list="#{'0':' 啟用','1':' 禁用'}" label="補貨功能" value="1" disabled="true"></s:radio>
    </s:if>
    <s:else>
    <s:radio name="agentStaffExt.replenishment" id="replenishment" list="#{'0':' 啟用','1':' 禁用'}" label="補貨功能" value="0"></s:radio>
    </s:else> 
...
<s:token/>
	...
	<s:form id="changeForm" action="%{pageContext.request.contextPath}/user/saveAgentStaff.action?searchAccount=%{searchAccount}&parentAccount=%{parentAccount}&searchUserStatus=%{searchUserStatus}&type=%{type}&searchType=%{searchType}&searchValue=%{searchValue}" target="_self">
	<input type="hidden" name="genAgentStaffExt.account" id="jq_genAgent_account"/>
	</s:form>
	<input type="hidden" id="jq_left_credit"  value="${selectGenAgent.availableCreditLine}"/>
	
	<input type="hidden" id="jq_max_rate" name="maxrate" value="<s:property value="#request.selectGenAgent.shareholderRate+#request.selectGenAgent.stockholderStaffExt.branchRate+#request.selectGenAgent.stockholderStaffExt.branchStaffExt.chiefRate"/>"/>
	
	
	<s:if test="#request.selectGenAgent.rateRestrict==1">
		<!-- 如果总代理有设置限制占成，取总代理的 -->
		<input type="hidden" id="jq_below_max_rate" name="belowmaxrate" value="${selectGenAgent.belowRateLimit}"/>
	</s:if>
	
	<s:elseif test="#request.selectGenAgent.stockholderStaffExt.rateRestrict==1">
		<!-- 否则取上上级股东设置的限制总成 -->
		<input type="hidden" id="jq_below_max_rate" name="belowmaxrate" value="${selectGenAgent.stockholderStaffExt.belowRateLimit}"/>
	</s:elseif>
	<s:else>
