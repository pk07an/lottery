...
<link type="text/css" rel="stylesheet" href="${pageContext.request.contextPath}/css/admin/index.css">
	<script language="javascript" src="${pageContext.request.contextPath}/js/jquery-1.7.2.min.js"></script>
		...
		function showResponse(jsonData, statusText)  { 
			self.location="${pageContext.request.contextPath}/user/queryGenAgentStaff.action?searchUserStatus=${searchUserStatus}&pageNo=${page.pageNo}&account=${searchAccount}&parentAccount=${parentAccount}&type=${type}&searchType=${searchType}&searchValue=${searchValue}";
...
<div class="content">
	<s:form name="sForm" action="%{pageContext.request.contextPath}/user/queryGenAgentStaff.action?account=%{searchAccount}&parentAccount=%{parentAccount}&type=%{type}">
			      ...
			      <tr>
			        ...
			        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
			          <tr>
			            ...
			            <td width="124" align="left" class="F_bold">總代理管理</td>
			            ...
			            <td class="t_right"width="129"><img src="${pageContext.request.contextPath}/images/admin/newadd.gif" width="14" height="13" /> <a href="${pageContext.request.contextPath}/user/saveGenAgentStaff.action?searchAccount=${searchAccount}&parentAccount=${parentAccount}&searchUserStatus=${searchUserStatus}&type=${type}&searchType=${searchType}&searchValue=${searchValue}">新增總代理</a></td>
			            </tr></table></td>
				...
				<tbody>
					...
					<s:iterator value="#request.page.result" status="branch">
						<tr onMouseOver="this.style.backgroundColor='#FFFFA2'"
							...
							<td><a href="#"><s:property value="stockholderStaffExt.account" escape="false" /></a></td>
							<td><a href="#"><s:property value="shareholderRate" escape="false" />%</a></td>
							...
							<s:action name="userTree" executeResult="true"  flush="false">
							   ...
							   <s:param name="userType">5</s:param>
							</s:action>
						...
						<img  src="${pageContext.request.contextPath}/images/admin/tuishui.gif"/>
						  <a class="btn_4" href="${pageContext.request.contextPath}/user/queryGenAgentcommission.action?qUserID=<s:property value="encodeId" escape="false"/>&searchAccount=${searchAccount}&parentAccount=${parentAccount}&searchUserStatus=${searchUserStatus}&type=${type}&searchType=${searchType}&searchValue=${searchValue}" target="_self">退水</a>
						 <img  src="${pageContext.request.contextPath}/images/admin/xiugai.gif"/>
							    <a class="btn_4" href="${pageContext.request.contextPath}/user/updateFindByGenAgent.action?account=<s:property value="account" escape="false"/>&searchAccount=${searchAccount}&parentAccount=${parentAccount}&searchUserStatus=${searchUserStatus}&type=${type}&searchType=${searchType}&searchValue=${searchValue}">修改</a>
						...
						<s:if test="flag==0"> <a href="##" name="userStatus" data="<s:property value="account" escape="false" />|0|5"><input type="button" value="啟用"/> </a></s:if> 
				     	    <s:if test="flag==1">
							<strong ><a href="##" name="userStatus" data="<s:property value="account" escape="false" />|1|5"><input type="button" value="停用" class="red"/></a></strong>
							</s:if> 
							<s:if test="flag==2">
									<strong ><a href="##" name="userStatus" data="<s:property value="account" escape="false" />|2|5"> <input type="button" value="凍結" class="blue"/></a></strong>
							</s:if>
				...
				<tbody>
					<tr>
						<td><s:if test="#request.page.totalCount>0">
								<lottery:paginate
									url="${pageContext.request.contextPath}/user/queryGenAgentStaff.action"
									param="" />
							</s:if></td>
					</tr>
				</tbody>
		...
		<tr>
		    <td height="35" background="${pageContext.request.contextPath}/images/admin/tab_19.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
		      <tr>
		        ...
		        <lottery:paginate
									url="${pageContext.request.contextPath}/user/queryGenAgentStaff.action"
									param="searchUserStatus=${searchUserStatus}&account=${searchAccount}&parentAccount=${parentAccount}&type=${type}"  recordType="個總代理帳號"/>
	<%-- 	        <table width="100%" border="0" cellspacing="0" cellpadding="0">
				<colgroup>
		          <col width="40%" />
				  <col width="20%" />
