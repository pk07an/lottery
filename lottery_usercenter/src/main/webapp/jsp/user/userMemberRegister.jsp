...
<script language="javascript">
	function checkShopsName() {
		...
		if (account != "") {
			var strUrl = '${pageContext.request.contextPath}/user/ajaxQueryMemberName.action';
	...
	function checkNum(){
		...
		if(eval(totalCredit) > eval(<%=moneyCredit%>)){
			alert("信用額度-不能大于-可用信用額度还剩-"+<%=moneyCredit%>);
			$("#totalCredit").val("");
		}
	}
...
<div class="content">
	<s:form id="sForm" action="%{pageContext.request.contextPath}/user/saveMember.action" method="post">
		<div class="main">
			<table cellspacing="0" cellpadding="0" border="0" width="100%"
				class="king2 xy pw">
				<tbody>
					<tr>
						<th colspan="2">会员注冊</th>
					</tr>
					<s:if test="userInfo.userType==2 || subAccountInfo.parentUserType==2">
						<tr bgcolor="#ffffff">
							<td class="tr">上级帳號</td>
							<td class="tl">
							<select id="userAccount" name="memberStaffExt.managerStaff.account" onchange="javascript:checkUserName();">
								 ...
								 <s:iterator value="agentStaffExtsSet">
					...
					<s:elseif test="userInfo.userType==3 || subAccountInfo.parentUserType==3">
						<tr bgcolor="#ffffff">
							...
							<select id="userAccount" name="memberStaffExt.managerStaff.account" onchange="javascript:checkUserName();">
								 ...
								 <s:iterator value="agentStaffExtsSet" var="a">
								 <s:if test="#request.selaccount==account">
								  	<option value="${account}"   selected="selected" >${account}</option>
								  </s:if>
								  <s:else>
					...
					<s:elseif test="userInfo.userType==5 || subAccountInfo.parentUserType==5">
						<tr bgcolor="#ffffff">
							<td class="tr">上级帳號</td>
							<td class="tl">
							<select id="userAccount" name="memberStaffExt.managerStaff.account" onchange="javascript:checkUserName();">
								 <option value="0">----</option>
								 <s:iterator value="genAgentStaffExt.agentStaffExtsSet" var="st">
								 <s:if test="#request.selaccount==account">
								  	<option value="${account}"   selected="selected" >${account}</option>
								  </s:if>
								  <s:else>
					...
					<s:else>
						...
						<td class="tl"><input type="text"
							value="${agentStaffExt.account}" name="memberStaffExt.managerStaff.account" readonly="readonly"></input></td>
						</tr>
					</s:else>
					<tr bgcolor="#ffffff">
					...
					<s:if test="agentStaffExt.flag==1">
					<tr bgcolor="#ffffff">
						<td class="tr">用户狀態</td>
						<td class="tl">
					...
					<s:elseif test="agentStaffExt.flag==2">
						<tr bgcolor="#ffffff">
							<td class="tr">用户狀態</td>
							<td class="tl">
					...
					<tr bgcolor="#ffffff">
						<td class="tr">代理占成</td>
						<td class="tl">
						<s:select list="rateCount" name="rate" id="rate" headerValue="不占成" headerKey="0" onchange="javascript:checkChief();">
						</s:select>
						</td>
					</tr>
					<tr bgcolor="#ffffff">
					...
					<s:if test="(userInfo.userType!=6 && userInfo.userType!=7) || subAccountInfo.parentUserType!=6">
					<tr bgcolor="#ffffff">
						<td class="tr">信用額度</td>
						<td class="tl"><input  onblur="javascript:checkNumType();"  maxlength="9" id="totalCredit"  onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" value="" name="memberStaffExt.totalCreditLine"
					...
					<s:elseif test="userInfo.userType ==6 || subAccountInfo.parentUserType==6">
						<tr bgcolor="#ffffff">
						<td class="tr">信用額度</td>
						<td class="tl"><input  onblur="javascript:checkNum();" maxlength="9" id="totalCredit"  onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" value="" name="memberStaffExt.totalCreditLine"
							class="w176" type="text" /></input><span  style="color: #FF0000;">可开信用額度还剩<input type="text" id="moneyTotalCredit" value="0" readonly="readonly"></span></td>
						</tr>
					</s:elseif>
					<tr bgcolor="#ffffff">
						<td class="tr">盘口</td>
