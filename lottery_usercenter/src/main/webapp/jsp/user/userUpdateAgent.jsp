...
<script language="javascript">
	...
	function checkChief1(){
		...
		var percentCount = $("#percentCount").val();
		var genAgentRate   = genAgentRates.split("%");
		$("#hgenAgentRate").val($("#genAgentRate1").val());
		var agent = agentRates.split("%");
		if(genAgentRate[0] != null){
			$("#agentRate").empty();
		}
...
$(document).ready(function() {
 ...
 $("#judge input[type!=hidden]").blur(function(){     
	 ...
	 if(eval(<%=rateFlag%>) == 0){
		...
		$("#agentRate").attr("disabled","true");//代理占成
		$("#genAgentRate1").attr("disabled","true");//代理占成
		$("#hgenAgentRate").val($("#genAgentRate1").val());
		$("#hgenAgentRate").val($("#genAgentRate").val());
		$("#hagentRate").val($("#agentRate").val());
...
<div class="content">
	<s:form id="sForm" action="%{pageContext.request.contextPath}/user/updateAgent.action" method="post">
		<div class="main">
			<table cellspacing="0" cellpadding="0" border="0" width="100%"
				class="king2 xy pw">
				<tbody>
					<tr>
						<th colspan="2">代理修改</th>
					</tr>
					<tr>
						<td class="tr">上级帳號</td>
						<td class="tl"><input type="text"
							value="${agentStaffExt.genAgentStaffExt.account}" name="agentStaffExt.genAgentStaffExt.account" readonly="readonly"></input>
							</td>
					</tr>
					<tr>
						<td class="tr" width="50%">帳號</td>
						<td class="tl" width="50%"><input type="text" value="${agentStaffExt.account}"
							maxlength="20" id="account" name="agentStaffExt.account" readonly="readonly"></input> <span
							id="accountAgain" style="color: #FF0000;"></span> <s:hidden
								id="verifyName" value="" /> <s:hidden
								name="agentStaffExt.userType" value="6" />
								<s:hidden name="agentStaffExt.managerStaffID" value="%{agentStaffExt.ID}"></s:hidden>
								</td>
					</tr>
					<tr>
						<td class="tr">密碼</td>
					...
					<s:if test="agentStaffExt.genAgentStaffExt.replenishment==1">
					<tr>
							<td class="tr">走飞</td>
							<td class="tl">
					...
					<s:else>
						<tr>
							<td class="tr">走飞</td>
							<td class="tl">
							<s:radio name="agentStaffExt.replenishment" list="#{'0':'允许','1':'禁止'}" label="走飞">
							</s:radio>
							</td>
						</tr>
					</s:else>
					<s:if test="agentStaffExt.genAgentStaffExt.flag==1">
					<tr>
						<td class="tr">用户狀態</td>
						<td class="tl">
						<s:radio name="agentStaffExt.flag" list="#{'0':'有效','2':'冻结'}" label="狀態" disabled="true">
						</s:radio>
					...
					<s:elseif test="agentStaffExt.genAgentStaffExt.flag==2">
						<tr>
							<td class="tr">用户狀態</td>
							<td class="tl">
							<s:radio name="agentStaffExt.flag" list="#{'0':'有效','1':'禁用'}" label="狀態" disabled="true">
							</s:radio>
							<s:radio name="agentStaffExt.flag" list="#{'2':'冻结'}" label="狀態" value="2">
							</s:radio>
							</td>
						</tr>
					</s:elseif>
					<s:else>
						...
						<td class="tr">用户狀態</td>
						<td class="tl">
						<s:radio name="agentStaffExt.flag" list="#{'0':'有效','1':'禁用','2':'冻结'}" label="狀態">
						</s:radio>
						</td>
					</tr>
					</s:else>
					<tr>
						<td class="tr">占成狀態</td>
						<td class="tl"><s:radio name="agentStaffExt.pureAccounted" id="pure" onchange="javascript:checkPure(this);"	
									list="#{'0':'纯占成','1':'含下级占成'}" label="狀態">
								</s:radio>
								<s:hidden value="%{agentStaffExt.pureAccounted}" id="hpureAccounted" name="hpureAccounted"></s:hidden>
						</td>
					</tr>
					<tr>
						<td class="tr">總代理占成</td>
						<td class="tl">
						<s:if test="agentStaffExt.genAgentRate==0">
							<s:if test="agentStaffExt.pureAccounted==0">
								<s:select id="genAgentRate"  list="genAgentRateCount"  headerValue="不占成" headerKey="0" onchange="javascript:checkChief();">
								</s:select>
								
								<s:select id="genAgentRate1" cssStyle="display:none" list="genAgentRateCount"  headerValue="不占成" headerKey="0" onchange="javascript:checkChief1();">
								</s:select>
								<s:hidden id="hgenAgentRate" name="hgenAgentRate" value="0"></s:hidden>
							</s:if>
							<s:else>
								<s:select id="genAgentRate" cssStyle="display:none" list="genAgentRateCount"  headerValue="不占成" headerKey="0" onchange="javascript:checkChief();">
								</s:select>
								
								<s:select id="genAgentRate1"  list="genAgentRateCount"  headerValue="不占成" headerKey="0" onchange="javascript:checkChief1();">
								</s:select>
								<s:hidden id="hgenAgentRate" name="hgenAgentRate" value="0"></s:hidden>
							</s:else>
						</s:if>
						<s:else>
						  <s:if test="agentStaffExt.pureAccounted==0">	
							<s:select id="genAgentRate"  list="genAgentRateCount"  headerValue="%{agentStaffExt.genAgentRate}%" headerKey="%{agentStaffExt.genAgentRate}%" onchange="javascript:checkChief();">
							</s:select>
							
							<s:select id="genAgentRate1" cssStyle="display:none" list="genAgentRateCount"  headerValue="%{agentStaffExt.genAgentRate}%" headerKey="%{agentStaffExt.genAgentRate}%" onchange="javascript:checkChief1();">
							</s:select>
							<s:hidden id="hgenAgentRate" name="hgenAgentRate" value="%{agentStaffExt.genAgentRate}%"></s:hidden>
						 </s:if>
						 <s:else>
						 	<s:select id="genAgentRate" cssStyle="display:none" list="genAgentRateCount"  headerValue="%{agentStaffExt.genAgentRate}%" headerKey="%{agentStaffExt.genAgentRate}%" onchange="javascript:checkChief();">
							</s:select>
							
							<s:select id="genAgentRate1"  list="genAgentRateCount"  headerValue="%{agentStaffExt.genAgentRate}%" headerKey="%{agentStaffExt.genAgentRate}%" onchange="javascript:checkChief1();">
							</s:select>
							<s:hidden id="hgenAgentRate" name="hgenAgentRate" value="%{agentStaffExt.genAgentRate}%"></s:hidden>
						 </s:else>
						</s:else>
					...
					<tr>
						...
						<td class="tl">
						<s:if test="agentStaffExt.agentRate==0">
							<s:select list="agentRateCount" id="agentRate" headerValue="不占成" headerKey="0" onchange="javascript:subRate();">
							</s:select>
							<s:hidden name="hagentRate" id="hagentRate" value=""></s:hidden>
						</s:if>
						<s:else>
							<s:select list="agentRateCount"  id="agentRate" headerValue="%{agentStaffExt.agentRate}%" headerKey="%{agentStaffExt.agentRate}%" onchange="javascript:subRate();">
							</s:select>
							<s:hidden name="hagentRate" id="hagentRate" value="%{agentStaffExt.agentRate}%"></s:hidden>
							</s:else>
						</td>
					</tr>
					<tr>
						<td class="tr">中文名字</td>
						<td class="tl"><input name="agentStaffExt.chsName"
							class="w176" type="text" value="${agentStaffExt.chsName}" /></input></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="tr">信用額度</td>
						<td class="tl"><input  onblur="javascript:checkNum('${agentStaffExt.totalCreditLine }');" id="totalCredit"  onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" value="${agentStaffExt.totalCreditLine }" name="agentStaffExt.totalCreditLine"
						...
						<s:hidden value="%{agentStaffExt.totalCreditLine}" id="checkCredit" ></s:hidden>
					</tr>
					<tr style="display: none">
						<td class="tr">可用信用額度</td>
						<td class="tl">
							<input id="available" name="agentStaffExt.availableCreditLine"
								class="w176" type="text" value="${agentStaffExt.availableCreditLine}" readonly="readonly"/></input>
						</td>
					</tr>
					<tr>
						<td class="tr">注册时间</td>
						<td class="tl"><s:date name="agentStaffExt.createDate"  format="yyyy-MM-dd HH:mm:ss" />
						<s:hidden name="agentStaffExt.createDate" value="%{agentStaffExt.createDate}"></s:hidden>
						</td>
					</tr>
				</tbody>
			</table>
