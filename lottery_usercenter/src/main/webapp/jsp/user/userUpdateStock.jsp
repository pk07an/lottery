...
<script language="javascript">
	...
	function checkChief(){
		...
		$("#hbranchRate").val($("#branchRate").val());
		if(branch[0] != null){
	...
	function checkChief1(){
		...
		var branch   = branchRates.split("%");
		var shareholder = shareholderRates.split("%");
		$("#hbranchRate").val($("#branchRate1 ").val());
		if(branch[0] != null){
			$("#shareholderRate").empty();
		}
		$("#shareholderRate").append("<option value='0'>不占成</option>");
	...
	function checkNum(moeryCount){
		...
		if(eval(tempCredit) > (eval(moeryCount)+eval(<%=moneyCredit%>))){
	...
	function checkNumType(){
		...
		if(eval(totalCredit) > eval(countMoney)){
			alert("信用額度-不能大于-可用信用額度-信用額度还剩-"+countMoney);
			$("#totalCredit").val("");
		}
	}
...
$(document).ready(function() {
 ...
 $("#judge input[type!=hidden]").blur(function(){     
	 ...
	 if(eval(<%=rateFlag%>) == 0){
		$("#branchRate").attr("disabled","true");    //分公司占成
		$("#shareholderRate").attr("disabled","true");//總监占成
		$("#branchRate1").attr("disabled","true");    //分公司占成
		$("#hbranchRate").val($("#branchRate").val());
		$("#hbranchRate").val($("#branchRate1").val());
		$("#hshareholderRate").val($("#shareholderRate").val());
...
<div class="content">
	<s:form id="sForm" action="%{pageContext.request.contextPath}/user/updateByStockholder.action" method="post">
		<div class="main">
			<table cellspacing="0" cellpadding="0" border="0" width="100%"
				class="king2 xy pw">
				<tbody>
					<tr>
						<th colspan="2">股東修改</th>
					</tr>
					<tr >
						<td class="tr">上级用户</td>
						<td class="tl"><input type="text"
							value="${stockholderStaffExt.branchStaffExt.account}" name="stockholderStaffExt.branchStaffExt.account" readonly="readonly"></input>
							</td>
					</tr>
					<tr >
						<td class="tr" width="50%">用户名</td>
						<td class="tl" width="50%"><input type="text" value="${stockholderStaffExt.account}"
							maxlength="20" id="account" name="stockholderStaffExt.account" readonly="readonly"></input> <span
							id="accountAgain" style="color: #FF0000;"></span> <s:hidden
								id="verifyName" value="" /> <s:hidden
								name="stockholderStaffExt.userType" value="4" />
								<s:hidden name="stockholderStaffExt.managerStaffID" value="%{stockholderStaffExt.ID}"></s:hidden>
								</td>
					</tr>
					<tr >
						<td class="tr">密码</td>
						<td class="tl"><input id="userPassword" type="password"
							value="" name="stockholderStaffExt.userPwd"></input></td>
					</tr>
					<s:if test="stockholderStaffExt.branchStaffExt.replenishment==1">
					<tr >
							<td class="tr">走飞</td>
							<td class="tl">
							<s:radio name="stockholderStaffExt.replenishment" list="#{'0':'允许'}" label="走飞" value="0" disabled="true">
							</s:radio>
							<s:radio name="stockholderStaffExt.replenishment" list="#{'1':'禁止'}" label="走飞" value="1">
							</s:radio>
							</td>
						</tr>
					</s:if>
					<s:else>
						<tr >
							<td class="tr">走飞</td>
							<td class="tl">
							<s:radio name="stockholderStaffExt.replenishment" list="#{'0':'允许','1':'禁止'}" label="走飞">
							</s:radio>
							</td>
						</tr>
					</s:else>
					<s:if test="stockholderStaffExt.branchStaffExt.flag==1">
					<tr >
						<td class="tr">用户狀態</td>
						<td class="tl">
						<s:radio name="stockholderStaffExt.flag" list="#{'0':'有效','2':'冻结'}" label="狀態" disabled="true">
						</s:radio>
						<s:radio name="stockholderStaffExt.flag" list="#{'1':'禁用'}" label="狀態" value="1">
						</s:radio>
						</td>
					</tr>
					</s:if>
					<s:elseif test="stockholderStaffExt.branchStaffExt.flag==2">
						<tr >
							<td class="tr">用户狀態</td>
							<td class="tl">
							<s:radio name="stockholderStaffExt.flag" list="#{'0':'有效','1':'禁用'}" label="狀態" disabled="true">
							</s:radio>
							<s:radio name="stockholderStaffExt.flag" list="#{'2':'冻结'}" label="狀態" value="2">
							</s:radio>
							</td>
						</tr>
					</s:elseif>
					<s:else>
						...
						<td class="tr">用户狀態</td>
						<td class="tl">
						<s:radio name="stockholderStaffExt.flag" list="#{'0':'有效','1':'禁用','2':'冻结'}" label="狀態">
						</s:radio>
						</td>
					</tr>
					</s:else>
					<tr >
						<td class="tr">占成狀態</td>
						<td class="tl"><s:radio name="stockholderStaffExt.pureAccounted" id="pure" onchange="javascript:checkPure(this);"	
									list="#{'0':'纯占成','1':'含下级占成'}" label="狀態">
								</s:radio>
								<s:hidden value="%{stockholderStaffExt.pureAccounted}" id="hpureAccounted" name="hpureAccounted"></s:hidden>
						</td>
					</tr>
					<tr >
						<td class="tr">分公司占成</td>
						<td class="tl">
						<s:if test="stockholderStaffExt.branchRate==0">
						<s:if test="stockholderStaffExt.pureAccounted==1">
						<s:select id="branchRate"  list="branchRateCount" cssStyle="display:none" headerValue="不占成" headerKey="0" onchange="javascript:checkChief();">
						</s:select>
						<s:select id="branchRate1" name="branchRate" list="branchRateCount"  headerValue="%{stockholderStaffExt.branchRate}%" headerKey="%{stockholderStaffExt.branchRate}%" onchange="javascript:checkChief1();" >
							</s:select>
						<s:hidden id="hbranchRate" name="hbranchRate" value="0"></s:hidden>
						</s:if>
						<s:else>
							<s:select id="branchRate"  list="branchRateCount"  headerValue="不占成" headerKey="0" onchange="javascript:checkChief();">
							</s:select>
							<s:select id="branchRate1" name="branchRate" cssStyle="display:none" list="branchRateCount"  headerValue="%{stockholderStaffExt.branchRate}%" headerKey="%{stockholderStaffExt.branchRate}%" onchange="javascript:checkChief1();" >
								</s:select>
							<s:hidden id="hbranchRate" name="hbranchRate" value="0"></s:hidden>
						</s:else>
						</s:if>
						<s:else>
						<s:if test="stockholderStaffExt.pureAccounted==0">
						
							<s:select id="branchRate" name="branchRate" list="branchRateCount"  headerValue="%{stockholderStaffExt.branchRate}%" headerKey="%{stockholderStaffExt.branchRate}%" onchange="javascript:checkChief();">
							</s:select>
							
							<s:select id="branchRate1" name="branchRate"  cssStyle="display:none"  list="branchRateCount"  headerValue="%{stockholderStaffExt.branchRate}%" headerKey="%{stockholderStaffExt.branchRate}%" onchange="javascript:checkChief1();" >
							</s:select>
							<s:hidden id="hbranchRate" name="hbranchRate" value="%{stockholderStaffExt.branchRate}%"></s:hidden>
						</s:if>
						<s:else>
							<s:select id="branchRate" name="branchRate" cssStyle="display:none" list="branchRateCount"  headerValue="%{stockholderStaffExt.branchRate}%" headerKey="%{stockholderStaffExt.branchRate}%" onchange="javascript:checkChief();">
							</s:select>
					...
					<tr >
						...
						<td class="tl">
						<s:if test="stockholderStaffExt.shareholderRate==0">
							<s:select list="shareholderRateCount" id="shareholderRate" headerValue="不占成" headerKey="0" onchange="javascript:subRate();">
							</s:select>
							<s:hidden name="hshareholderRate" id="hshareholderRate" value="0"></s:hidden>
						</s:if>
						<s:else>
							<s:select list="shareholderRateCount"  id="shareholderRate" headerValue="%{stockholderStaffExt.shareholderRate}%" headerKey="%{stockholderStaffExt.shareholderRate}%" onchange="javascript:subRate();">
							</s:select>
							<s:hidden name="hshareholderRate" id="hshareholderRate" value="%{stockholderStaffExt.shareholderRate}%"></s:hidden>
							</s:else>
						</td>
					</tr>
					<tr >
						<td class="tr">中文名字</td>
						<td class="tl"><input name="stockholderStaffExt.chsName"
							class="w176" type="text" value="${stockholderStaffExt.chsName}" /></input></td>
					</tr>
					<tr >
						<td class="tr">股東已用信用額度</td>
						<td class="tl">
					...
					<tr >
						<td class="tr">信用額度</td>
						<td class="tl"><input  onblur="javascript:checkNum('${stockholderStaffExt.totalCreditLine }');" id="totalCredit"  onafterpaste="this.value=this.value.replace(/\D/g,'')" onkeyup="this.value=this.value.replace(/\D/g,'')" value="${stockholderStaffExt.totalCreditLine }" name="stockholderStaffExt.totalCreditLine"
						...
						<s:hidden value="%{stockholderStaffExt.totalCreditLine }" id="checkCredit" ></s:hidden>
					</tr>
					<tr  style="display:none">
						<td class="tr">可用信用額度</td>
						<td class="tl">
							<input id="available" name="stockholderStaffExt.availableCreditLine"
								class="w176" type="text" value="${stockholderStaffExt.availableCreditLine}" readonly="readonly" style="border:none"/></input>
						</td>
					</tr>
					<tr >
						<td class="tr">注册时间</td>
						<td class="tl"><s:date name="stockholderStaffExt.createDate"  format="yyyy-MM-dd HH:mm:ss" />
						<s:hidden name="stockholderStaffExt.createDate" value="%{stockholderStaffExt.createDate}"></s:hidden>
						</td>
					</tr>
				</tbody>
			</table>
