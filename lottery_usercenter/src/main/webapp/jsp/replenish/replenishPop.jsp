...
function replenishSubmitChief(){
	...
	var obj=
		$.ajax({url:queryUrl,async:false,
			type : "POST",
			data : {typeCode:$("#playFinalType").val(),plate:$("#rPlate").val(),lotteryType:$("#lotteryType").val(),
...
<%@ include  file="/jsp/admin/frame/topSub.jsp"%><!--加载判断用户信息，包括子帐号的信息判断  -->
	...
	<div id="replenishShow"
		...
		<s:form id="replenishForm" action="ajaxReplenishSubmit.action" namespace="replenish" method="post">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="king"
				...
				<tr>
					<td width="38" style="height:30px">類型</td>
					<td width="105" class="t_left"><strong class="blue" id="rFinalType"></strong></td>
				</tr>
				<tr>
					<td style="height:30px">會员</td>
					<td class="t_left">
					    <s:select id="replenishUserId" name="replenish.replenishUserId" list="#request.mapC" listKey="key" listValue="value" headerKey="0"/>
					</td>
				</tr>
				<tr>
					<td style="height:30px">賠率</td>
					<td class="t_left"><input id="rOdds" class="b_g" name="replenish.odds"
						style="width: 76px;" /></td>
				</tr>
				<tr>
					<td style="height:30px">金額</td>
				...
				<tr>
					...
					<%if(isSub){ 
      	               if(replenishment !=null){%>
					 	  <input id="rBtnSubmit" type="button" class="sl btn3" value=" 補 出 " onClick="replenishShowSubmit()"/> 
					   <%}else{ %>
					      <input id="rBtnSubmit" type="button" class="sl btn3" value=" 補 出 " disabled="disabled" title="該功能已被禁用,請聯系上級"/> 
					   <%} %>
					<%}else { %>
					      <input id="rBtnSubmit" type="button" class="sl btn3" value=" 補 出 " onClick="replenishShowSubmit()"/> 
					<%} %>
					   <input id="retbtn" type="button" class="sl btn3" value=" 取 消 " /></td>
				</tr>
			</table>
			<!-- 存儲 彈出框是香港還是重慶廣東 -->
		...
		<s:form id="replenishFormOther" action="ajaxReplenishSubmit.action" namespace="replenish" method="post">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="king"
				...
				<tr>
					<td>金額</td>
					<td class="t_left">
					<input id="moneyOther" name="replenish.money" class="b_g"
						style="width: 76px;" onBlur="onlyNumber(this);"/></td>
				</tr>
				<tr>
					<td>限額</td>
				...
				<tr>
					...
					<%if(isSub){ 
      	               if(replenishment !=null){%>
					 	  <input id="rBtnSubmitOther" type="button" class="sl btn3" value=" 補 出 " onClick="replenishShowSubmitOther()"/> 
					   <%}else{ %>
					      <input id="rBtnSubmitOther" type="button" class="sl btn3" value=" 補 出 " disabled="disabled" title="該功能已被禁用,請聯系上級"/> 
					   <%} %>
					<%}else { %>
					      <input id="rBtnSubmitOther" type="button" class="sl btn3" value=" 補 出 " onClick="replenishShowSubmitOther()"/> 
					<%} %>
	...
	<!-- 總監赔率修改开始-->
	<!-- 手写设置赔率 -->
<div id="oddsShow" style="background-color:#fff; display:none;">
   <s:form id="oddForm" action="" namespace="replenish" method="post" onkeydown= "#">
     <table width="100%" border="0" cellpadding="0" cellspacing="0" class="king" style="width:136px;">
		    ...
		    <tr><td colspan="2" class="even"><div class="tj">
		           <span><%if(isSub){ 
      	               if(odd !=null){%>
					 	  <input id="rBtnSubmit" type="button" class="btn td_btn" value=" 確定  " onClick="opOdd()"/> 
					   <%}else{ %>
					      <input id="rBtnSubmit" type="button" class="btn td_btn" value=" 確定  " disabled="disabled" title="該功能已被禁用,請聯系上級"/> 
					   <%} %>
					<%}else { %>
					      <input id="rBtnSubmit" type="button" class="btn td_btn" value=" 確定  " onClick="opOdd()"/> 
					<%} %>
