...
com.npc.lottery.common.Constant,com.npc.lottery.periods.entity.NCPeriodsInfo,
...
<%
Map<String,ReplenishVO> oddMap=(Map<String,ReplenishVO>)request.getAttribute("NCShopOddsMap");
    ...
    $(".rOp").on("click", function () {
      ...
      <%}else{%>
    	...
    	if ($("#searchType").val()=="1"){
		    ...
		    $div.show();
	   	    $("#rOdds").focus();
	   	    getTypeCodeState("NCBALL",typeCode,'');
    	}
    	<%}%> 
    });  
...
<% String ballNum="1"; String chinaBall="";String preName="FIRST";%>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_FIRST==subType'><%ballNum="1";chinaBall="第一球";preName="FIRST"; %></s:if>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_SECOND==subType'><%ballNum="2";chinaBall="第二球";preName="SECOND"; %></s:if>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_THIRD==subType'><%ballNum="3"; chinaBall="第三球";preName="THIRD";%></s:if>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_FORTH==subType'><%ballNum="4";chinaBall="第四球"; preName="FORTH";%></s:if>
<s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_FIFTH==subType'><%ballNum="5";chinaBall="第五球"; preName="FIFTH";%></s:if>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_SIXTH==subType'><%ballNum="6";chinaBall="第六球"; preName="SIXTH";%></s:if>
 <s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_SEVENTH==subType'><%ballNum="7";chinaBall="第七球";preName="SEVENTH"; %></s:if>
<s:if test='@com.npc.lottery.common.Constant@LOTTERY_NC_SUBTYPE_BALL_EIGHTH==subType'><%ballNum="8";chinaBall="第八球"; preName="EIGHTH";%></s:if> 
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" id="oddTable">
      ...
      <td valign="top" align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="king">
		  ...
		  <tr>
		    ...
		    <s:iterator value="#request.NCShopOddsMap">
        <tr <s:if test='#request.RunningPeriods.state=="2" && value.fpState=="0"'> class="noneTr"</s:if><s:else>class="even3Tr"</s:else> onmouseover="changeColorToYL(this,this.className)" onmouseout="changeColorToOld(this)">
            <td <s:if test='value.fpState=="1"'> class="even3"</s:if><s:else>class="even"</s:else>>
      ...
      <td width="160" valign="top" align="left"><!-- 右边统计开始--><jsp:include page="/jsp/replenish/ncRightBar.jsp" /></td>
		  </tr>
		</table>
        <!-- 表格内容结束 一行五列-->
		</td>
...
$(document).ready(function(){
	var double = context+'/replenish/ajaxtwoSideAndDragonRank.do?from=NC';
	$.ajax({url:double,dataType:"html",type:"POST",success:function(data){ $("#doubleStat").html(data);doubleStat=data;}});
});
</script>
