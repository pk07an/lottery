...
<script type="text/javascript">
   function changeTime(){
	   if($("#isPet").val()!="true"){
		   document.sForm.action = "${pageContext.request.contextPath}/statreport/settledRelenishDetailedEric.action";
	   }
	   $("#sForm").submit();
...
<body>
<div id="content">
<s:form name="sForm" action="settledDetailedListEric.action" id="sForm" namespace="statreport">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <s:hidden name="lotteryType"/>
  <s:hidden name="playType"/>
  <s:hidden name="userID"/>
  <input type="hidden" value='<s:date name="bettingDateStart" format="yyyy-MM-dd"/>' name="bettingDateStart"/>
  <input type="hidden" value='<s:date name="bettingDateEnd" format="yyyy-MM-dd"/>' name="bettingDateEnd"/>
  <s:hidden name="userType"/>
  <s:hidden name="periodsNum"/>
  <s:hidden name="selectTypeRadio"/>
  <s:hidden name="parentUserType"/>
  <input type="hidden" name="isPet" id="isPet" value="<s:property value='#request.isPet' escape='false'/>"/>
  <tr>
      ...
      <tr>
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <s:if test="#request.mapTj==null">
            	<img src="${pageContext.request.contextPath}/images/admin/close.gif" width="10" height="10" /><a href="#" onClick='window.close()'>关闭</a>
            </s:if>
            <s:else>
	            <b class="blue">選擇日期：</b>
				<s:select id="searchDate" name="searchDate" list="#request.mapTj" listKey="key" listValue="value" cssClass="blue" onchange="changeTime()"  ></s:select>	
			</s:else>
            </td>
            </tr></table></td>
	        ...
	        <s:if test="null == #request.page.result || 0 == #request.page.result.size()">
	            ...
	            <s:iterator value="#request.page.result" status="">
		            <tr onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
		              ...
		              <s:if test="winState==4 || winState==5">
		              		<td class="t_right" style="text-decoration:line-through;color:red;"><s:property value="moneyDis" escape="false" /></td>
		              </s:if><s:else>
			              <td class="t_right"><s:property value="moneyDis" escape="false" /></td>
		              </s:else>
		              <s:if test="winState==4 || winState==5">
		              		<td class="t_right" style="text-decoration:line-through;color:red;"><s:property value="winAmountDis" escape="false" /></td>
		              </s:if><s:else>
			              <td class="t_right"><s:property value="winAmountDis" escape="false" /></td>
		              </s:else>
		           ...
		           <%} %> 
		            ...
		            <s:if test="winState==4">
		              		<td class="t_right" style="color:red;">注銷</td>
		              </s:if><s:elseif test="winState==5">
			              <td class="t_right" style="color:red;">注銷</td>
		              </s:elseif><s:else>
			              <td nowrap class="t_right"><strong><s:property value="rateWinAmountDis" escape="false" /></strong><br />
			                <s:property value="rateBackWaterDis" escape="false" /></td>
		              </s:else>
		            </tr>
	            <!-- 内容结束 -->
	          </s:iterator>
      ...
      <tr>
        ...
        <td align="center">
            <s:if test="#request.isPet=='true'">
	            <lottery:paginate url="${pageContext.request.contextPath}/statreport/settledDetailedListEric.action" 
	            param="playType=${playType}&bettingDateStart=${bettingDateStart}&bettingDateEnd=${bettingDateEnd}&lotteryType=${lotteryType}&selectTypeRadio=${selectTypeRadio}&userType=${userType}&userID=${userID}&periodsNum=${periodsNum}&parentUserType=${parentUserType}&searchDate=${searchDate}" recordType=" 筆注單"/>
            </s:if><s:else>
	            <lottery:paginate url="${pageContext.request.contextPath}/statreport/settledRelenishDetailedEric.action" 
		            ...
		            &userType=${userType}&userID=${userID}&periodsNum=${periodsNum}&searchDate=${searchDate}" recordType=" 筆注單"/>
            </s:else>
          </td>
