...
<body>
	<div id="content">
		<s:form action="updateNCOpenOdds.action" namespace="admin" onsubmit="changeValueOnSubmit();">
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0"
										...
										<s:iterator value="openOddsList" status="st">
											<tr  onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
												<s:if test="oddsType.trim()=='NC_BALL_FIRST'">
													<td>第一球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_SECOND'">
													<td>第二球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_THIRD'">
													<td>第三球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_FORTH'">
													<td>第四球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_FIFTH'">
													<td>第五球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_SIXTH'">
													<td>第六球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_SEVENTH'">
													<td>第七球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_BALL_EIGHTH'">
													<td>第八球</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_DX'">
													<td>1-8大小</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_DS'">
													<td>1-8單雙</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_WDX'">
													<td>1-8尾數大小</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_HSDS'">
													<td>1-8合數單雙</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_FW'">
													<td>1-8方位</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_1-8_ZFB'">
													<td>1-8中發白</td>
												</s:if>
												<%-- <s:if test="oddsType.trim()=='NC_1-8_ZFB_B'">
													<td>1-8中發白(白)</td>
												</s:if> --%>
												<s:if test="oddsType.trim()=='NC_ZHDX'">
													<td>總和大小</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_ZHDS'">
													<td>總和單雙</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_ZHWSDX'">
													<td>總和尾數大小</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_DOUBLESIDE_LH'">
													<td>龍虎</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_RX2'">
													<td>任選二</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_R2LZ'">
													<td>選二連組</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_RX3'">
													<td>任選三</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_R3LZ'">
													<td>選三前組</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_RX4'">
													<td>任選四</td>
												</s:if>
												<s:if test="oddsType.trim()=='NC_RX5'">
													<td>任選五</td>
												</s:if>
