...
public class NCBallOddsAction extends BaseAdminLotteryAction {
	...
	private static Logger logs = Logger.getLogger(NCBallOddsAction.class);
	 ...
	 */
	private String subType = Constant.LOTTERY_NC_SUBTYPE_BALL_FIRST;
	...
	public String updateOdds() {
		...
		// 未開盤不能修改
    	if(null == this.getNCRunningPeriodsNum() ||"".equals(getNCRunningPeriodsNum()))
    		return SUCCESS;
		...
		while (names.hasMoreElements()) {
			...
			if (ele.indexOf("NC_BALL_") != -1
					|| ele.indexOf("NC_DOUBLESIDE_") != -1) {
		...
		shopOddsLogic.setRunningPeriodsNum(this.getNCRunningPeriodsNum());
...
//        BigDecimal autoMoney=openOdd.getAutoOdds();//自动降赔率
		...
		log.setType("2");
		try{
			log.setPeriodsNum(this.getNCRunningPeriodsNum());
