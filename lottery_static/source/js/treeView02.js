...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var ctrlValue = this.getAttribute(source, "ctrlValue");
  var ctrlChecked = this.getAttribute(source, "ctrlChecked");
  var ctrlDisabled = this.getAttribute(source, "ctrlDisabled");
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(eval(node.ctrl)){      //{}中部分由xuStanly_cn增加于2007.8.28
      ...
      "' type='"+ node.ctrlType +"' name='"+ node.ctrlName +"' value="+ node.ctrlValue;
      if(node.ctrlClick) HTML += " onclick='"+ this.name +".getPath(\""+ node.ctrlType +"\",\""+node.path+"\")'"
      HTML += " style='border:none;'";
      if(eval(node.ctrlChecked)){
        HTML +=" checked";
      }
      if(eval(node.ctrlDisabled)){
        HTML +=" disabled";
      }
      HTML += ">";
  }
    ...
    "onclick=\"return "+ this.name +".nodeClick('"+ id +"')\" ";
      ...
      if(node.rtClick != "") HTML += "oncontextmenu=\""+ node.rtClick +"\"";        //由xuStanly_cn增加于2007.1.26
