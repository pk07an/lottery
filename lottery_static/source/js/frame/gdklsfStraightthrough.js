...
function fp()
{
	...
	$("#GDKLSF_STRAIGHTTHROUGH_RX2").html("-");
	$("#GDKLSF_STRAIGHTTHROUGH_RX3").html("-");
	$("#GDKLSF_STRAIGHTTHROUGH_RX4").html("-");
	$("#GDKLSF_STRAIGHTTHROUGH_RX5").html("-");
	$("#GDKLSF_STRAIGHTTHROUGH_R2LZ").html("-");
	$("#GDKLSF_STRAIGHTTHROUGH_R3LZ").html("-");
...
function displayBetInfoOnLeft()
{
	...
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_RX2')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_RX2").text();
		number=2;
		type="任選二";
		if(selecedSize<2)
	   {alert('"任選二"最少必須選擇兩個號碼');return false;}
	}
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_R2LZ')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_R2LZ").text();
		number=2;
		type="選二連組";
		if(selecedSize<2)
			{
	...
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_RX3')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_RX3").text();
		number=3;
		type="任選三";
		if(selecedSize<3)
			{
	...
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_R3LZ')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_R3LZ").text();
		number=3;
		type="選三前組";
		if(selecedSize<3)
			{
	...
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_RX4')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_RX4").text();
		number=4;
		type="任選四";
		if(selecedSize<4)
			{
	...
	if(radioType=='GDKLSF_STRAIGHTTHROUGH_RX5')
	{
		odds=$("#GDKLSF_STRAIGHTTHROUGH_RX5").text();
		number=5;
		type="任選五";
		if(selecedSize<5)
			{
...
function createFormForSubmit(resultArr,balls,playType,odds,winQuto,betQuotas) 
{
	...
	var betNum="<p>";
	 ...
	 form = $("#ajaxform",frameDocument('leftFrame'));
	    form.html("");
	    form.attr('action',"klsfStraightthroughSub.do");
...
$(document).ready(function() {
	...
	$("#res").click(function() {
	 ...
	 parent.topFrame.options = { 
		        ...
		        url:       context+ '/gdklsf/submitBetLM.json',          
		        type:      'post',       
		        dataType:  'json'       		     
		    }; 
