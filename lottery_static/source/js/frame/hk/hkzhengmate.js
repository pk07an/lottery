...
$(document).ready(function() {
	...
	$("#quicksubmit").click(function() {
		...
		form.html("");
		zt=$("#ZT").val();
		form.attr('action',"ajaxZTMSub.action");
		form.attr('method','post'); 
		subType = $('<s:hidden name="subType" value="%{#parameters.subType[0]}"/>');
		ztType = $('<input type=hidden name="ZT" value='+zt+' />');
		form.append(ztType);
		form.append(subType);
	 ...
	 $("#ztmSubmit").click(function() {
			...
			for(i=0;i<betPrices.size();i++)
			{
				...
				parameter=parameter+"*"+ betPrices[i].name+"_"+zt+"@"+betPrices[i].value;
			}
			//alert(sum);
	 ...
	 parent.topFrame.options = { 
		        ...
		        url:      context+'/member/ajaxhkZTMSub.do',
		        type:      'post',        
		        dataType:  'json'         
		    }; 	
	 var options = { 
...
form = $("#ajaxform",frameDocument('leftFrame'));
form.html("");
form.attr('action',"ajaxhkZTMSub.do");
form.attr('method','post');
...
form.append(hidOdds);
var betTypeValue=inputName+"_"+zt;
inputBetType = ("<input type='hidden' name='checkType' id='checkType' value='"+betTypeValue+"' />");
//inputBetType.attr('value',inputName+"_"+zt);
form.append(inputBetType);
var json=ajaxGetTopWinPrice(inputName+"_"+zt);
