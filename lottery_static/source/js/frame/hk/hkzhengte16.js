...
/*	  
	 ...
	 $("#betBallTable a").click(function() {
			...
			var odd=$(this).text();
			var name=$(this).parent().parent("tr").find("td:eq(0)").text();
			
			var maCode=$(this).parent().next().children().attr("name");
			var maName="";
			if(maCode.indexOf("ZM1")!=-1)
				maName="正碼一";
			if(maCode.indexOf("ZM2")!=-1)
				maName="正碼二";
			if(maCode.indexOf("ZM3")!=-1)
				maName="正碼三";
			if(maCode.indexOf("ZM4")!=-1)
				maName="正碼四";
			if(maCode.indexOf("ZM5")!=-1)
				maName="正碼五";
			if(maCode.indexOf("ZM6")!=-1)
				maName="正碼六";
			...
			createSubmitForm(inputName,maName+"  "+name,odd); 
	 ...
	 parent.topFrame.options = { 
		        ...
		        url:       context+'/member/ajaxhkZM1To6Sub.action',
		        type:      'post',        
		        dataType:  'json'         
		    }; 	
...
form = $("#ajaxform",frameDocument('leftFrame'));
form.html("");
form.attr('action',"ajaxhkZM1To6Sub.do");
