...
$(document).ready(function() {
	initPageBall();
	 $("#bbsubmit").click(function() {
			//alert("test");
			var betPrices=$("#betBallTable input[type='text'][value!='']");
			if(betPrices.size()==0)
			{
				alert("请输入投注金額");
				return false;
...
/*	 $('#price').live('keyup', function() {
	 ...
	 parent.topFrame.options = { 
		        ...
		        url:      context+'/member/ajaxhkBBSub.do',
		        type:      'post',        
		        dataType:  'json'         
		    }; 
	...
	/*	    $('#ajaxform',frameDocument('leftFrame')).submit(function() { 
		    	var typeCode=$('#price',frameDocument('leftFrame')).attr("name");
...
form = $("#ajaxform",frameDocument('leftFrame'));
form.html("");
form.attr('action',"ajaxhkBBSub.do");
...
{
	$("#reddan").html(changeNumToBall(reddan));
	$("#redshuang").html(changeNumToBall(redshuang));
	$("#bluedan").html(changeNumToBall(bluedan));
	$("#blueshuang").html(changeNumToBall(blueshuang));
	$("#greendan").html(changeNumToBall(greendan));
	$("#greenshuang").html(changeNumToBall(greenshuang));
	$("#redda").html(changeNumToBall(redda));
	$("#redxiao").html(changeNumToBall(redxiao));
	$("#blueda").html(changeNumToBall(blueda));
	$("#bluexiao").html(changeNumToBall(bluexiao));
	$("#greenda").html(changeNumToBall(greenda));
	$("#greenxiao").html(changeNumToBall(greenxiao));
