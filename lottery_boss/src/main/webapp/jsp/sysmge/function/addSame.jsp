...
<script language="javascript">
    ...
    function submitFunc(){
        ...
        if(confirm("确认新增同级节点吗？")){

            document.addSameForm.submit();
    ...
    //校验输入的相关数据信息
    function checkData(){
        var name = document.addSameForm.funcName.value;
        var code = document.addSameForm.funcCode.value;
        if(null == code || 0 == code.length){
           alert("请输入功能代碼！");
           document.addSameForm.funcCode.focus();
           return false;
        }
        
        if(null == name || 0 == name.length){
            alert("请输入功能名稱！");
            document.addSameForm.funcName.focus();
            return false;
        } 
...
<div id="main">
    ...
    <div id="cnt_lt">
        ...
        <div id="cnt_tblcontent">   
        <s:form name="addSameForm" action="/sysmge/function/submitAddSame.action">
        <table width="100%"  class="cnt_tblbody">
         <tr>
             <td class="tab-head" width="15%" align="right">
				功能代碼：
         ...
         <tr>
             ...
             <td class="td-input" >
                <s:property value="#request.function.parentFunc.funcName"/>
                <input type="hidden" name="parentFuncID" value="<s:property value="#request.function.parentFunc.ID"/>" />
