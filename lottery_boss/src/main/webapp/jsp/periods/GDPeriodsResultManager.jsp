...
function setupPriods() {
	$("#sForm").attr("action","${pageContext.request.contextPath}/boss/gdPeriodsInfo.action?periodsType=periodsType_gd");
	$("#sForm").submit();
}
function delPriods() {
	window.location.href="${pageContext.request.contextPath}/boss/delGDPeriodsInfo.do";
}
</script>
<body>
<div class="content">
<s:form name="sForm" id="sForm">
	<div class="main">
	  ...
	  <tbody>
 		    <tr>
				<input type="button" name="btn1" id="button3" value="手动生成广东盤期" onclick="setupPriods()" />
				&nbsp;&nbsp;&nbsp;<input type="button" value="删除广东盤期" onclick="delPriods()" />
			</tr>
	    <tr>
	      <th width="5%">序號</th>
	      <th width="15%">期數</th>
	          ...
	          <tr>
	            <td>
		            <s:if test="#request.page.totalCount>0">			      
					      <lottery:paginate url="${pageContext.request.contextPath}/boss/queryGDResultPeriods.action" param="periodsType=periodsType"/>
					</s:if>
				</td>
	          </tr>
	        </tbody>
