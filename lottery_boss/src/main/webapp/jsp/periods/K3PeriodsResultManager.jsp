...
function setupPriods() {
	$("#sForm").attr("action","${pageContext.request.contextPath}/boss/k3PeriodsInfo.action?periodsType=periodsType_k3");
	$("#sForm").submit();
}
function delPriods(){
	$("#sForm").attr("action","${pageContext.request.contextPath}/boss/delK3PeriodsInfo.action?periodsType=periodsType_k3");
	$("#sForm").submit();
}
</script>
<body>
...
<s:form name="sForm" id="sForm">
	<div class="main">
	  ...
	  <tbody>
 		    <tr>
				<input type="button" name="btn1" id="button3" value="手动生成快3盤期" onclick="setupPriods()" />
				&nbsp;&nbsp;&nbsp;<input type="button" name="btn1" id="button3" value="删除当天快3盤期" onclick="delPriods()" />
			</tr>
	    <tr>
	      <th width="5%">序號</th>
	      <th width="15%">期數</th>
	          ...
	          <tr>
	            <td>
		            <s:if test="#request.page.totalCount>0">			      
					      <lottery:paginate url="${pageContext.request.contextPath}/boss/queryK3ResultPeriods.action" param="periodsType=periodsType"/>
					</s:if>
				</td>
	          </tr>
	        </tbody>
