...
<div class="main">
  ...
  <tbody>
    ...
    %>      
      ...
      <%if("5".equals(periond.getState().trim())){ %>
          <td bgcolor="#FF0000"><a href="${pageContext.request.contextPath}/boss/queryNCLotResult.action?periodsNum=<%=periondNum %>"><%=periondNum %></a></td>
      <%}else{ %>
          <td><a href="${pageContext.request.contextPath}/boss/queryNCLotResult.action?periodsNum=<%=periondNum %>"><%=periondNum %></a></td>
      <%}%>
      ...
      <% if("6".equals(periond.getState().trim()) ){ %><td width="18%" colspan="5"><strong class="blue">官方停售，獎期取消</strong></td><%}else{ %>
	      ...
	      <td width="3%">
	      <%if(NCRule.ZHX(ballList)){ %>小 <%} else if(NCRule.ZHDA(ballList)){%><span class="red">大</span><%} else{%>和<%} %>
	      </td>
	   ...
	   <%if(NCRule.ZHWX(ballList)){ %>尾小 <%} else{%><span class="red">尾大</span><%} %>
	      ...
	      <td><%if(NCRule.HU(ballList)){ %>虎 <%} else{%><span class="red">龍</span><%} %></td>
      <%} %>
      <td> 
 ...
 <% }%>   
  ...
  <s:if test="#request.page.totalCount>0">
	...
	<lottery:paginate url="${pageContext.request.contextPath}/boss/enterLotResultHistoryBoss.action" param="subType=NC&state=${state}" css="lotPage"/>
</s:if>
